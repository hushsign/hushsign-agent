...
public abstract class AbstractModemDriver
{
	...
	public AbstractModemDriver(ModemContext modem)
	{
		...
		{
			logger.warn("no modem.properties on classpath, using built-in defaults", e);
		}
		this.modem = modem;
	}
	...
	private String getResponse() throws TimeoutException, IOException, NumberFormatException, InterruptedException
	{
		...
		while (true)
		{
			...
			if (line.startsWith("+CLIP:"))
			{
				...
				// HushSign: inbound-call callback plumbing removed with the Service machinery.
				// The AT behaviour (hang up on +CLIP) is preserved.
				logger.debug("incoming call from " + callerId + " hung up");
				Common.countSheeps(Integer.valueOf(getModemSettings("wait_unit")));
				continue;
			}
